...
@section('title', 'Kelola Akun Panitia - Sistem Event')

@section('content')
    <!-- Header Section -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-8 border-b border-slate-800">
        <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs font-semibold tracking-wide mb-2 shadow-sm">
                Manajemen Pengguna
            </div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">Kelola Akun Panitia</h1>
            <p class="text-slate-400 text-xs sm:text-sm mt-1">Daftar akun panitia yang memiliki akses untuk mengelola acara.</p>
        </div>
    ...
    <div class="rounded-3xl bg-slate-950/40 border border-slate-800/80 p-6 sm:p-8 backdrop-blur-xl shadow-xl overflow-hidden relative">
            ...
            <table class="w-full text-xs sm:text-sm text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-800 text-slate-400">
                        ...
                        <th class="py-3.5 px-4 font-semibold uppercase tracking-wider text-[11px]">Email</th>
                    ...
                    @forelse($panitias as $panitia)
                        <tr class="group hover:bg-slate-900/40 transition-colors duration-200">
                            ...
                            <td class="py-4 px-4 text-right">
                                <form method="POST" action="{{ route('panitia.destroy', $panitia) }}" class="inline" onsubmit="return confirm('Yakin hapus akun ini?')">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="py-12 text-center text-slate-500 text-xs">
                                Belum ada akun panitia yang tercatat.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="mt-6 pt-4 border-t border-slate-900">
            @if(isset($panitias) && method_exists($panitias, 'links'))
                {{ $panitias->links() }}
            @endif
        </div>
    </div>
