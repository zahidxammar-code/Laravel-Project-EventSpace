...
@section('title', 'Buat Acara Baru - Sistem Event')
...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <div class="pb-6 mb-6 border-b border-slate-800">
            ...
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">Buat Acara Baru</h1>
            <p class="text-slate-400 text-xs sm:text-sm mt-1">Isi formulir berikut untuk mempublikasikan kegiatan baru ke sistem.</p>
        </div>

        <!-- Form Card Container -->
        <div class="rounded-3xl bg-slate-950/40 border border-slate-800/80 p-6 sm:p-8 backdrop-blur-xl shadow-xl">
            <form method="POST" action="{{ route('acara.store') }}" class="space-y-5">
                @csrf

                <!-- Nama Acara -->
                <div>
                    <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Nama Acara</label>
                    <input type="text" name="nama" value="{{ old('nama') }}" placeholder="Contoh: Workshop Laravel Tailwind 2026" 
                ...
                <div>
                    ...
                    <select name="kategori_id" class="w-full bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 text-xs sm:text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 hover:border-slate-700 transition-all duration-200" required>
                        ...
                        @foreach($kategoris as $kategori)
                            <option value="{{ $kategori->id }}" class="bg-slate-900 text-white" @selected(old('kategori_id') == $kategori->id)>
                                {{ $kategori->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4" placeholder="Tuliskan detail atau sinopsis acara di sini..." 
                        class="w-full bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 text-xs sm:text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 hover:border-slate-700 transition-all duration-200">{{ old('deskripsi') }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" name="lokasi" value="{{ old('lokasi') }}" placeholder="Contoh: Aula Utama Sekolah / Lab Komputer" 
                        class="w-full bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 text-xs sm:text-sm text-white focus:outline-none focus:border-blue-500 transition" required>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="kapasitas" value="{{ old('kapasitas', 50) }}" placeholder="50" 
                            class="w-full bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 text-xs sm:text-sm text-white focus:outline-none focus:border-blue-500 transition" required>
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 text-xs sm:text-sm text-white focus:outline-none focus:border-blue-500 transition">
                            <option value="akan_datang" class="bg-slate-900" @selected(old('status') === 'akan_datang')>Akan Datang</option>
                            <option value="berlangsung" class="bg-slate-900" @selected(old('status') === 'berlangsung')>Berlangsung</option>
                            <option value="selesai" class="bg-slate-900" @selected(old('status') === 'selesai')>Selesai</option>
                        </select>
